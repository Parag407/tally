...
Handles upload, preview, validation, and XML generation for purchase vouchers.
"""
...
async def upload_purchase_file(
    ...
    """
    Upload an Excel/CSV file for purchase voucher processing.
    """
    ...
    if use_ai:
        mapping = await ai_based_map(columns, "purchase")
    else:
        mapping = rule_based_map(columns, "purchase")

    records = dataframe_to_records(df, mapping)
    cleaned, errors = validate_purchase(records)
...
async def generate_purchase_xml_endpoint(
    file: UploadFile = File(...),
    use_ai: bool = Query(False),
    auto_fixed: bool = Query(False),
    purchase_ledger: str = Query("Purchase Account", description="Purchase ledger name in Tally"),
):
    """
    Generate Tally XML from uploaded purchase file.
    """
    if not file.filename:
        raise HTTPException(status_code=400, detail="No file uploaded")
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": f'attachment; filename="purchase_voucher.xml"'
        },
    )


@router.post("/validate")
async def validate_purchase_file(
    file: UploadFile = File(...),
    use_ai: bool = Query(False),
):
    """Validate purchase file and return only errors."""
    if not file.filename:
        raise HTTPException(status_code=400, detail="No file uploaded")
