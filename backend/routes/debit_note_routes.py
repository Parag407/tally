"""
Debit Note Voucher Routes
Handles upload, preview, validation, and XML generation for Debit Note vouchers.
"""
...
@router.post("/upload")
async def upload_debit_note_file(
    file: UploadFile = File(...),
    use_ai: bool = Query(False, description="Use AI for column mapping"),
):
    """
    Upload an Excel/CSV file for Debit Note voucher processing.
    ...
    if use_ai:
        mapping = await ai_based_map(columns, "debit_note")
    else:
        mapping = rule_based_map(columns, "debit_note")

    records = dataframe_to_records(df, mapping)
    cleaned, errors = validate_debit_note(records)
...
async def generate_debit_note_xml_endpoint(
    file: UploadFile = File(...),
    use_ai: bool = Query(False),
    auto_fixed: bool = Query(False),
):
    """
    Generate Tally Prime XML from uploaded Debit Note file.
    """
    if not file.filename:
        raise HTTPException(status_code=400, detail="No file uploaded")
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": 'attachment; filename="debit_note_voucher.xml"'
        },
    )


@router.post("/validate")
async def validate_debit_note_file(
    file: UploadFile = File(...),
    use_ai: bool = Query(False),
):
    """Validate Debit Note file and return only errors."""
    if not file.filename:
        raise HTTPException(status_code=400, detail="No file uploaded")
